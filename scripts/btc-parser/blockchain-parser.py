...
def write_graph_to_files():
    ...
    client = ArangoClient(hosts='http://' + host + ':' + port)
    sys_db = client.db('_system', username=username, password=password)

    if not sys_db.has_database(arango_db):
        sys_db.create_database(arango_db)

    db = client.db(arango_db, username=username, password=password)

    if db.has_graph(arango_db):
        blockchain = db.graph(arango_db)
    else:
        blockchain = db.create_graph(arango_db)

    bc = db.graph(arango_db)
